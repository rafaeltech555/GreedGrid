...
| **OS folder drop into cell** | Drag a folder (or file) from the OS file manager and drop it onto a grid cell. A floating **DropMenu** appears and lets you choose **File** (opens a File Browser panel rooted at that folder) or **Terminal** (opens a terminal with its working directory set to that folder). If a file is dropped instead of a folder, its parent directory is used. Dropping onto an already-occupied cell shows a confirmation dialog before overwriting. When multiple items are dropped at once, only the first path is used by design. |
