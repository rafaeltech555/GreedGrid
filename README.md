...
| **Merge panel preservation** | `mergeCells` no longer blindly keeps the top-left cell's panel regardless of what it holds. The store now branches on how many selected cells contain a panel: **0 panels** → merged cell is empty; **exactly 1 panel** → that panel is preserved in place regardless of which cell it occupied (`instanceId` is kept intact, so a running Terminal PTY carries over without even detaching); **2+ panels** → a `MergeConflictDialog` appears listing each candidate with its type glyph, label, and key config detail (Terminal → cwd, Web → URL, File → path), letting the user choose which panel survives; the rest are closed via `onDestroy`. Fixes panels silently vanishing when the selection's top-left cell was empty but an adjacent cell held a running panel. Consistent with the existing panel-preservation conventions (preset remap, drag-move swap). |
| **Panel maximize** | Each panel's control bar gains a `⛶` button. Pressing it expands that panel to fill the entire grid while hiding (but not killing) all other panels — Terminal PTYs, System Monitor sampling, and Web pages keep running in the background. Press `Esc` or click the button again to restore the original layout. The native webview overlay (wry + gtk::Overlay) resizes/hides in sync. Switching a layout preset or loading a workspace automatically exits maximize and restores the grid. |
| **Terminal idle reminder** | When a terminal panel's foreground command finishes and the user has not yet looked back at that cell, an amber slow-pulse animation fires: a per-panel border glow, an in-panel "idle" badge, a Toolbar status chip (click to jump to the panel), and a system tray icon that changes colour (first tray integration in the project). The tray menu exposes "Show window" and "Quit" — required on Cinnamon / libappindicator where a tray icon without a menu is invisible. Foreground-process detection uses the pty's `process_group_leader()`. The idle state clears as soon as the user types, clicks, focuses the window, clicks the chip, or chooses "Show window" from the tray menu. |
